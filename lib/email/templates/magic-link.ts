...
export function renderMagicLinkEmail({ url }: { url: string }) {
  const subject = "Your Convene sign-in link";
  const text = `Sign in to Convene by opening this link (valid 15 minutes):\n\n${url}\n\nIf you didn't request this, you can ignore the email.`;
  const html = `
<!doctype html><html><body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:520px;margin:0 auto;padding:32px 24px;color:#111">
  <h1 style="font-size:22px;font-weight:600;margin:0 0 16px">Sign in to Convene</h1>
  ...
  <p style="margin:0 0 24px"><a href="${url}" style="display:inline-block;background:#111;color:#fff;text-decoration:none;padding:12px 20px;border-radius:8px;font-size:15px">Open Convene</a></p>
